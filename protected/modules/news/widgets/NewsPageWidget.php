<?php
class NewsPageWidget extends CWidget
{
    public $tpl='default';
    ...
    public function run()
    {
    	...
    	if ($id_next == 0)
    	{
			...
			$sql = "SELECT * FROM miad_news ORDER BY time DESC LIMIT ".$first.",".$total;
			$command=$connection->createCommand($sql);
    		...
    		$connection=Yii::app()->db;
    		$sql = "SELECT * FROM {{news}} where id=".$id_next;
			$command=$connection->createCommand($sql);
			$rows=$command->queryAll();
			if (sizeof($rows) == 0) throw new CHttpException(404,'The requested page does not exist.');
        ...
        $this->render('NewsPage/' . $this->tpl, array('rows'=>$rows, 'count'=>$count, 'page_num'=>$num, 'page_now'=>$page_str, 'id_next'=>$id_next));
    }
}